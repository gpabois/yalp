...
lazy_static! {
    static ref TABLE: Result<LrTable<'static, 'static>, LrParserError<'static, 'static>> = LrTable::build::<1, _>(&GRAMMAR);
}
...
pub fn parse_grammar(stream: TokenStream) -> Result<GrammarInput, Error> {
    ...
    let table = TABLE.as_ref().map_err(|err| Error::ParserError(err.clone()))?;
