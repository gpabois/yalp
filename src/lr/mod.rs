...
use crate::{lexer::{traits::Lexer, LexerError}, parser::{traits::Ast, Parser}, Grammar, ItemSetId, RuleId, RuleReducer, RuleSet, Symbol};
...
impl std::fmt::Display for LrParserError<'_, '_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LrParserError::ShiftReduceConflict { state, symbol, conflict } => write!(f, "shift/reduce conflict for symbol {}, (state: #{}) [{:?}]", symbol.id, state, conflict),
            LrParserError::UnsupportedLrRank => write!(f, "cannot build LR table for K > 1."),
            LrParserError::LexerError(error) => write!(f, "{}", error),
            LrParserError::MissingAction(state_id, symbol) => write!(f, "missing action for terminal {} (state #{})", symbol, state_id),
            LrParserError::MissingGoto(state_id, symbol) => write!(f, "missing goto for non-terminal {} (state #{})", symbol, state_id),
            LrParserError::UnknownSymbol(symbol_id) => write!(f, "unknown symbol {}", symbol_id),
            LrParserError::UnexpectedSymbol { expected, got } => write!(f, "unexpected symbol {}, expecting {}", got, expected),
        }
    }
}
...
impl<'sid, 'sym, 'table, 'reducers, Node> LrParser<'sid, 'sym, 'table, 'reducers, Node> 
    where Node: Ast
{
    pub fn new(grammar: &'sym Grammar<'sid>, table: &'table Table<'sid, 'sym>, reducers: &'reducers [RuleReducer<'sid, Node>]) -> Self {
        if reducers.len() != grammar.iter_rules().count() {
            panic!("the number of reducers must match the number of grammar rules.")
        }

        Self{rules: RuleSet::new(&grammar), table, reducers}
    }
}
    ...
    where Node: Ast
{
    ...
    where Self::Ast: From<L::Token> {
            ...
            match action {
                ...
                Action::Reduce(rule_id) => {
                    ...
                    let ast = {
                        ...
                        drained.as_slice().iter().zip(rule.rhs.iter()).map(|(node, expected_symbol)| {
                            if node.symbol_id() != expected_symbol.id {
                                Err(LrParserError::UnexpectedSymbol { expected: expected_symbol.id, got: node.symbol_id().to_string() })
                            } else {
                                Ok(())
                            }
                        }).collect::<Result<_, _>>()?;
                    ...
                    if ast.symbol_id() != rule.lhs.id {
                        return Err(LrParserError::UnexpectedSymbol { expected: rule.lhs.id, got: ast.symbol_id().to_string() })
                    }
        ...
        Err(
            LexerError::unexpected_end_of_stream(lexer.current_location()).into()
        )
    }
}
...
#[cfg(test)]
mod tests {
    use crate::{ast::AstNode, fixtures::{fixture_lr0_grammar, fixture_lr1_grammar}, lexer::fixtures::lexer_fixture_lr0, parser::Parser as _};
    ...
    pub fn test_lr0_parser() {
        let grammar = fixture_lr0_grammar().expect("cannot build LR(0) grammar.");
        let table = Table::build::<0>(&grammar).expect("cannot build table");
    
        let mut lexer = lexer_fixture_lr0("1 + 1 * 0".chars());
        let parser = LrParser::<AstNode<'_>>::new(&grammar, &table, &[
            AstNode::reduce,
            AstNode::reduce,
            AstNode::reduce,
            AstNode::reduce,
